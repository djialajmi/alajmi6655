...
def check_quota(file_size, chat_id) -> bool:
    remain, _, ttl = VIP().check_remaining_quota(chat_id)
    if file_size > remain:
        refresh_time = current_time(ttl + time.time())
        err = f"تتجاوز الحصة لديك {sizeof_fmt(remain)} متبقي, " \
              f"لكنك تريد تنزيل مقطع فيديو {sizeof_fmt(file_size)} في الحجم. \n" \
              f"جرب مرة اخري في {ttl} ثواني({refresh_time})"
        logging.warning(err)
        ...
        for path in resp["filepath"]:
            ...
            if mime in default_type:
                if not can_convert_mp4(path, bot_msg.chat.id):
                    ...
                    bot_msg._client.send_message(bot_msg.chat.id, "لا يمكن تحويل الفيديو الخاص بك إلى steaming.")
                    break
                edit_text(bot_msg, f"{current_time()}: تحويل {path.name} إلي mp4. من فضلك انتظر.")
...
def add_instagram_cookies(url: "str", opt: "dict"):
    if url.startswith("https://www.instagram.com"):
        ...
        path = pathlib.Path(__file__).parent.joinpath("instagram.com_cookies.txt").as_posix()
        print(path)
        logging.info(f"Instagram cookies added {path}")
