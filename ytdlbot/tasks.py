...
def forward_video(url, client, bot_msg):
    ...
    try:
        ...
        bot_msg.edit_text(f"تم التنزيل بنجاح!✅✅✅")
...
def direct_normal_download(bot_msg, client, url):
    ...
    if ENABLE_VIP:
        ...
        try:
            ...
            length = 0
        if remain < length:
            bot_msg.reply_text(f"اسف لقد تعديت الحد الاقصي للكوتة.\n")
            return
    ...
    with tempfile.TemporaryDirectory(prefix="ytdl-") as f:
        ...
        for chunk in req.iter_content(1024 * 1024):
            text = tqdm_progress("يتم التنزيل...", length, downloaded)
...
def normal_audio(bot_msg, client):
    ...
    with tempfile.TemporaryDirectory(prefix="ytdl-") as tmp:
        ...
        status_msg.edit_text("جارٍ إرسال الصوت الآن ...")
        client.send_chat_action(chat_id, 'upload_audio')
        for f in resp["filepath"]:
            client.send_audio(chat_id, f)
        status_msg.edit_text("✅ التحويل إكتمل.")
