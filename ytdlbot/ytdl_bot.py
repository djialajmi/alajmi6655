...
def download_handler(client: "Client", message: "types.Message"):
    ...
    if not re.findall(r"^https?://", url.lower()):
        ...
        message.reply_text("أعتقد انه يجب ان ترسل لي رابط.", quote=True)
        return
        # TODO
    ...
    if re.findall(r"^https://www\.youtube\.com/channel/", VIP.extract_canonical_link(url)):
        message.reply_text("تم تعطيل تنزيل القناة الآن. من فضلك أرسل لي رابط فيديو فردي.", quote=True)
    ...
    try:
        ...
        bot_msg = message.reply_document(f, caption=f"حسن الانتظار! الرجاء الانتظار {e.x} ثانية ...."
                                                    f"ستبدأ وظيفتك تلقائيًا", quote=True)
        f.close()
        client.send_message(OWNER, f"قيضان انتظر! 🙁 {e.x} ثانية....")
        time.sleep(e.x)
...
def send_method_callback(client: "Client", callback_query: types.CallbackQuery):
    ...
    callback_query.answer(f"تم تعيين نوع الإرسال الخاص بك على {callback_query.data}")
...
def download_resolution_callback(client: "Client", callback_query: types.CallbackQuery):
    ...
    callback_query.answer(f"تم تعيين جودة التنزيل الافتراضية على {callback_query.data}")


@app.on_callback_query(filters.regex(r"convert"))
def audio_callback(client: "Client", callback_query: types.CallbackQuery):
    callback_query.answer(f"جاري التحويل إلى صوت ... يرجى الانتظار بصبر")
...
def owner_local_callback(client: "Client", callback_query: types.CallbackQuery):
    ...
    callback_query.answer(f"تم تغيير وضع التنزيل إلى {callback_query.data}")
...
def periodic_sub_check():
    ...
    for cid, uids in vip.group_subscriber().items():
        ...
        if video_url:
            ...
            for uid in uids:
                try:
                    bot_msg = app.send_message(uid, f"{video_url} يتم تنزيل ...", disable_web_page_preview=True)
...
By @SuperAbdo, VIP mode: {ENABLE_VIP}, Distribution: {ENABLE_CELERY}
